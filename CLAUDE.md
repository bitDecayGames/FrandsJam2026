...
- `db` — enables in-game debug buttons/tools in `PlayState` (Rock, Big Rock, Pepper, Waders, End Round, Dog, Rocket, Potion, Fish, Bait, Gravity). Items not consumed on use. Fish state labels + scare radius circles shown. Also enables lobby auto-ready when another player joins. Used by `watch_net.sh`.
...
### Gravity Bomb
Press V to drop at the player's feet (server validates via `use_gravity_bomb`). For 8s, all players within 112px (lake-sized area — full strength inside, zero outside) get an additive 40px/s pull toward the bomb — applied inside `Simulation.tickPlayer` (via `simulation.gravityWell`) so server, client prediction, and reconciliation replay all match, and normal collision (water, bushes, world) still applies. Remote player interpolation passes `applyWellForces=false` since server positions already include the pull. Players inside the radius get their cast auto-reeled and can't start a new cast (`Player.isGravityAffected()`); outside the radius fishing is unaffected. Visual is `entities/GravityBomb.hx` — pulsing core + swirl particles spiraling inward. One bomb active at a time; cleared on round reset.

### Time of Day & Night Lighting (shared/GameLogic.hx, source/shaders/TimeOfDayShader.hx, source/ui/TimeOfDayHUD.hx)
See [docs/time-of-day.md](docs/time-of-day.md) for the full system. Summary: server-authoritative clock (0-24h, starts at noon, stands still — `TIME_NORMAL_RATE = 0`, only `set_time` moves it); `set_time` fast-forwards at 6 game-hours/sec (sundial HUD buttons: Morning 7:30, Day 12:00, Evening 19:00, Night 0:00); server broadcasts `time_sync` {hour, rate} every 5s, clients integrate locally. `TimeOfDayShader` (camera filter) grades the environment via keyframes — noon = identity, morning soft yellow (~4000K), afternoon warm, evening orange-red (~2200K), night = **pitch black**. At night the only light is the shader's amber light field: a steady candle around the local player (120px, 240px in hot mode, eased) plus up to 16 guide glows (burning bushes 90px full-strength, remote burning players, faint item/dog markers) collected by `PlayState.collectNightGlows()`. Lights compose via screen blend (never `max()` — gradient ridge). OpenFL can't parse array uniforms, hence unrolled `uGlow0..uGlow15`. All screen-space HUD lives on a separate transparent `uiCamera` (created before `loadLevel()`) so the filter never dims it — new HUD elements need `cameras = [uiCamera]`. Night (`GameLogic.isNight()`, 21:00-6:00): seagulls stop spawning (existing ones fly off naturally) and cloud shadows fade via `CloudShadow.visibilityFactor`.
