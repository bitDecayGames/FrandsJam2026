...
class LLMDebugBridge {
	...
	static var _walkTarget:{x:Float, y:Float, thresh:Float, active:Bool, framesUsed:Int} = {x: 0, y: 0, thresh: 2, active: false, framesUsed: 0};
	...
	static function _walkTo(x:Float, y:Float, ?threshold:Float):String {
		...
		if (dist <= thresh) {
			return Json.stringify({arrived: true, x: roundFloat(p.x), y: roundFloat(p.y), framesUsed: 0});
		}
		...
		_releaseAll();
		if (dx > thresh) _heldButtons.set("RIGHT", true);
		if (dx < -thresh) _heldButtons.set("LEFT", true);
		if (dy > thresh) _heldButtons.set("DOWN", true);
		if (dy < -thresh) _heldButtons.set("UP", true);

		_stepFrames(maxFrames);
		_walkTarget = {x: x, y: y, thresh: thresh, active: true, framesUsed: 0};
		return Json.stringify({walking: true, targetX: roundFloat(x), targetY: roundFloat(y), maxFrames: maxFrames});
	}
